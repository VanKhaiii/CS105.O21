...
function init() {
    ...
    // Điểm để nhìn vào - thiết lập VRP
    ...
    let time = 0;
    // Hàm cập nhật để xử lý animation và biến đổi
    function update() {
        ...
        // Phép tịnh tiến đi thẳng theo chiều z
        coin.position.z += 0.01 / 2;
        
        // Tịnh tiến dao động quanh 1 điểm
        time += 0.01;
        const amplitude = 3.0; // Biên độ dao động
        cylinder.position.x = Math.sin(time) * amplitude;
        
        // Cập nhật điểm nhìn của camera
        lookAtVec.x += 0.01;
        camera.lookAt(lookAtVec);
        
        // Gọi hàm cập nhật ở khung hình tiếp theo
        controls.update();
        
        // Kết xuất cảnh và camera
